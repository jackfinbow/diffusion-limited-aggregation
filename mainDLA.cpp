...
void drawFuncs::introMessage() {
	...
	cout << "  m to set multiple runs of the simulation with different seeds" << endl;
	cout << "  0 to print the fractal dimension to console" << endl;
}
...
void drawFuncs::handleKeypress(unsigned char key, int x, int y) {
	...
	case 'g':
		cout << "go" << endl;
		sys->openFile("fracdim_");  // open file for writing
		cout << "setting seed " << sys->seed << endl;
		sys->setRunning();
	...
	case 'u':
		...
		break;
	case 'm':
		sys->multiple_seeds = 1;  // run with multiple seeds
		break;
	case '0':
		sys->printFracDim(0);  // print fractal dimension to console
		break;
	}
