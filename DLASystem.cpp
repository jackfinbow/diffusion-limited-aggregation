...
void DLASystem::Update() {
	...
	// if multiple simulations selected, with different seeds
	else if ((seed < 101) && (multiple_seeds == 1))  {
		// once DLA simulation has finished
		if (numParticles == endNum && lastParticleIsActive == 0)  {
			closeFile();
			Reset();
			seed++;  // increment seed
			setSeed(seed);  // pass new seed to rnd
			openFile("fracdim_");
			setRunning();
			glutPostRedisplay();  // redraw the window
			Update();
			nP = 0;  // reset numParticles_array element specifier
		}
		Update();
	}
	...
	// print number of particles and cluster size to file
	if (numParticles == numParticles_array[nP] && lastParticleIsActive == 0)  {
		printFracDim();  // print to file
		nP++;  // increment array element specifier
	}
...
int DLASystem::checkStick() {
	Particle *lastP = particleList[numParticles - 1];
	int result = 0;
	float p = 0.6;  // value of finite probability of sticking (0<=p<=1)
	float m = 1000.0;  // upper cut-off of random number generator (must be float to prevent integer divide)
	// loop over 4 possible directions to check if occupied by neighbour
	for (int i = 0; i < 4; i++) {
		double checkpos[2];
		setPosNeighbour(checkpos, lastP->pos, i);
		// if the neighbour is occupied...
		if (readGrid(checkpos) == 1)  {
			int x = rgen2.randomInt(m) + 1;  // random int between 1 and m
			float stick_prob = x / m;  // produce sticking probability between 0 and 1
			if (stick_prob <= p)  {
				result = 1;  // ... then stick
			}
		}
...
// calculate the fractal dimension and print to console (0) or file ()
// function overloading
void DLASystem::printFracDim(int iffile)  {
	// print numParticles, clusterRadius and fractal dimension to console
	cout << "Number of particles = " << numParticles << ", Size = " << clusterRadius << endl;
	cout << "Fractal dimension = " << dimension() << endl;
}
void DLASystem::printFracDim()  {
	// print numParticles and clusterRadius to file
...
DLASystem::DLASystem(Window *set_win) {
	...
	win = set_win;
	numParticles = 0;
	endNum = 1000;  // set max number of particles in cluster
	seed = 6;  // set initial seed
	setSeed(seed);  // pass seed to rnd
	nP=0;
